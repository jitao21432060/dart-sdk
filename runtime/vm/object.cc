...
ClassPtr Class::New(IsolateGroup* isolate_group, bool register_class) {
  ...
  result.set_is_declared_in_bytecode(false);
  result.set_binary_declaration_offset(0);
#else
...
#if defined(DART_PRECOMPILED_RUNTIME) && !defined(DART_DYNAMIC_RUNTIME)
  return untag()->code_ != StubCode::LazyCompile().ptr();
#else
  return untag()->code_ != StubCode::LazyCompile().ptr() &&
         untag()->code_ != StubCode::InterpretCall().ptr();
#endif  // defined(DART_PRECOMPILED_RUNTIME)
}
...
void Function::AttachBytecode(const Bytecode& value) const {
  ...
  StorePointer(&untag()->bytecode_, value.ptr());
...
#if defined(DART_PRECOMPILED_RUNTIME) && !defined(DART_DYNAMIC_RUNTIME)
  return function->untag()->code_ != StubCode::LazyCompile().ptr();
#else
  return function->untag()->code_ != StubCode::LazyCompile().ptr() &&
         function->untag()->code_ != StubCode::InterpretCall().ptr();
#endif  // !defined(DART_PRECOMPILED_RUNTIME)
}
...
void Function::ClearBytecode() const {
  ...
  UNREACHABLE();
#else
  StorePointer(&untag()->bytecode_, Bytecode::null());
#endif  // defined(DART_PRECOMPILED_RUNTIME)
}
...
#else
  result.SetInstructionsSafe(StubCode::InterpretCall());
#endif
...
#if defined(DART_DYNAMIC_RUNTIME)
    if (!is_static()&&!IsConstructor() && is_declared_in_bytecode() &&
      !HasCode() && !is_abstract()) {
#else
    if (!is_static() && !IsConstructor()) {
#endif
...
void Library::AddMetadata(const Object& declaration,
  ...
  UNREACHABLE();
#else
  // bytecode is negative in metadata map
  // kernel is positive
...
ObjectPtr Library::GetMetadata(const Object& declaration) const {
  ...
  if (kernel_offset < 0) {
    ...
    Object& metadata =
        Object::Handle(kernel::BytecodeReader::ReadAnnotation2(*this,
                                                               kernel_offset));
    return metadata.ptr();
  } else {
    // aot
...
ArrayPtr Library::GetExtendedMetadata(const Object& obj, intptr_t count) const {
  ...
  return kernel::BytecodeReader::ReadExtendedAnnotations2(*this, kernel_offset,
                                                          count);
#endif  // defined(DART_PRECOMPILED_RUNTIME)
}
...
//  StorePointer(&untag()->bytecode_component_, bytecode_component.ptr());
  untag()->set_bytecode_component(bytecode_component.ptr());
}
...
#endif  // !defined(DART_PRECOMPILED_RUNTIME) || defined(DART_DYNAMIC_RUNTIME)
...
#else
  data.SetAt(data_pos + CodeIndexFor(num_args_tested), Object::smi_zero());
#endif
...
#if defined(DART_DYNAMIC_RUNTIME)
void ICData::EnsureHasReceiverCheck2(intptr_t receiver_class_id,
                                    const Function& target,
                                    intptr_t count,
                                    StaticTypeExactnessState exactness) const {
  SafepointMutexLocker ml(IsolateGroup::Current()->type_feedback_mutex());
  ...
  AddReceiverCheck2(receiver_class_id, target, count, exactness);
}
#endif
...
#if defined(DART_DYNAMIC_RUNTIME)
void ICData::AddReceiverCheck2(intptr_t receiver_class_id,
                                  const Function& target,
                                  intptr_t count,
                                  StaticTypeExactnessState exactness) const {
  ASSERT(!target.IsNull());
  ...
  data.SetAt(data_pos, Smi::Handle(Smi::New(receiver_class_id)));
  if (!target.HasCode()) {
    data.SetAt(data_pos + CountIndexFor(kNumArgsTested), target);
    data.SetAt(data_pos + TargetIndexFor(kNumArgsTested),
               Smi::Handle(Smi::New(count)));
    if (is_tracking_exactness()) {
        data.SetAt(data_pos + ExactnessIndexFor(kNumArgsTested),
                   Smi::Handle(Smi::New(exactness.Encode())));
    ...
    data.SetAt(data_pos + EntryPointIndexFor(kNumArgsTested), target);
  }
  // Multithreaded access to ICData requires setting of array to be the last
  // operation.
  set_entries(data);
...
void ICData::FlateMegamorphicCache(const MegamorphicCache& cache) const {
  NoSafepointScope no_safepoint;
  ArrayPtr raw_data = entries();
  int count = NumberOfChecks();
  for (int i = 0; i < count; i++) {
    intptr_t data_pos = i * TestEntryLength();
    const Smi& class_id =
        Smi::Handle(Smi::RawCast(raw_data->untag()->data()[data_pos]));
    data_pos += NumArgsTested();
    const Object& obj = Object::Handle(raw_data->untag()->data()[data_pos]);
    if (obj.IsFunction()) {
      cache.EnsureContains(class_id, obj);
    } else if (obj.IsCode()) {
      cache.EnsureContains(class_id, Smi::Handle(
              Smi::FromAlignedAddress(Code::EntryPointOf(
                                      Code::RawCast(obj.ptr())))));
    } else {
      continue;
    }
  }
...
BytecodePtr Bytecode::New(uword instructions,
  ...
  Bytecode& result = Bytecode::Handle();
  {
    ...
    ObjectPtr raw = Object::Allocate(Bytecode::kClassId, size, Heap::kOld,
                                     Bytecode::ContainsCompressedPointers());
    NoSafepointScope no_safepoint;
    result ^= raw;
...
#else
  if (!AbstractType::Handle(bound()).IsNull() &&
      AbstractType::Handle(bound()).IsRecursive(trail)) {
#endif
    return true;
  }
  return false;
