...
void StubCodeCompiler::GenerateInterpretCallStub(Assembler* assembler) {
  ...
  __ Stop("Not using interpreter")
#else
  SPILLS_LR_TO_FRAME(__ EnterFrame((1 << R1) | (1 << R2) | (1 << R3) |
                (1 << R4) | (1 << R5) | (1 << FP) | (1 << LR), 0));
  __ set_constant_pool_allowed(true);
  // Reserve space for locals.
  __ AddImmediate(SP, 0);
#endif
  ...
  // first.
  RESTORES_LR_FROM_FRAME(__ LeaveFrame((1 << R1) | (1 << R2) | (1 << R3) |
          (1 << R4) | (1 << R5) | (1 << FP) | (1 << LR)));
...
void StubCodeCompiler::GeneratePatchFuntionCallStub(Assembler* assembler) {
  compiler::Label functionPath, done;
  __ EnterStubFrame();
  ...
  __ CompareClassId(R0, kICDataCid, TMP);
  __ b(&functionPath, NE);
  __ PopList((1 << R4) | (1 << R9));
  __ mov(TMP, compiler::Operand(R0));
  __ ldr(R0, compiler::FieldAddress(TMP,
                compiler::target::ICData::owner_offset()));
  __ ldr(R4, compiler::FieldAddress(TMP,
                compiler::target::CallSiteData::arguments_descriptor_offset()));
  __ b(&done);

  __ Bind(&functionPath);
  __ PopList((1 << R4) | (1 << R9));
  __ Bind(&done);
  __ LeaveDartFrame();
  __ Ret();
