...
class Frame {
  ...
  late VariableDeclaration closureVar;
  ...
  VariableDeclaration getSyntheticVar(String name) {
    if (name == ":async_completer") {
      print("=====func is:${function}");
      if (function.toString() == "LocaleNamesLocalizationsDelegate.load") {
        print("=====111123");
      }
    }
...
class _Allocator extends RecursiveVisitor<Null> {
  ...
  void _enterScope(TreeNode node) {
    ...
    if (_currentScope!.frame != _currentFrame) {
      ...
      if (_currentScope!.parent != null) {
        _currentFrame!.contextLevelAtEntry =
            _currentScope!.parent!.contextLevel;
      }
    ...
    final int? parentContextLevel = _currentScope!.parent != null
        ? _currentScope!.parent!.contextLevel
        : -1;
  ...
  void _allocateTemp(TreeNode node, {int count: 1}) {
    ...
    if (_currentScope!.tempsUsed + count > _currentFrame!.temporaries.length) {
      ...
      final int newSlots = (_currentScope!.tempsUsed + count) -
          _currentFrame!.temporaries.length;
  ...
  @override
  visitInstanceSet(Expression node) {
    _visit(node, temps: 1);
  }
  ...
  @override
  visitInstanceGet(Expression node) {
    int numTemps = 0;
    if (locals.directCallMetadata != null) {
      final directCall = locals.directCallMetadata[node];
      if (directCall != null && directCall.checkReceiverForNull) {
