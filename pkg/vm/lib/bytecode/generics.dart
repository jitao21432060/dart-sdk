...
List<DartType> getTypeParameterTypes(List<TypeParameter> typeParameters) {
  ...
  final types = new List<DartType>.filled(
      typeParameters.length,
      new TypeParameterType(typeParameters[0],
          TypeParameterType.computeNullabilityFromBound(typeParameters[0])));
...
bool isUncheckedCallLocal(Member? interfaceTarget, LocalFunctionInvocation node,
    StaticTypeContext staticTypeContext) {
  if (interfaceTarget == null) {
    // Dynamic call cannot be unchecked.
    return false;
  ...
  DartType receiverStaticType = node.functionType.returnType;
  if (receiverStaticType is InterfaceType) {
    final typeArguments = receiverStaticType.typeArguments;
    if (typeArguments.isEmpty) {
      return true;
...
bool isUncheckedClosureCallForLocalFunction(LocalFunctionInvocation node,
        StaticTypeContext staticTypeContext, BytecodeOptions options) =>
    node.name.text == 'call' && !options.avoidClosureCallInstructions;
