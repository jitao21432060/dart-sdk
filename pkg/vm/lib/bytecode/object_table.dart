  ...
  void readContents(BufferedReader reader) {
    ...
    if (hasNamedParams) {
      namedParams = new List<NameAndType>.generate(
          ...
          (_) => new NameAndType(reader.readPackedObject() as NameHandle,
              reader.readPackedObject() as TypeHandle));
    } else {
...
class _ConstObjectHandle extends ObjectHandle {
  ...
  void writeContents(BufferedWriter writer) {
    switch (tag) {
      ...
      case ConstTag.kMap:
        {
          final entries = value as List<Map<ObjectHandle, ObjectHandle>>;
          writer.writePackedObject(type!);
          writer.writePackedMap(entries);
        }
        break;
      case ConstTag.kList:
        {
        ...
        value = Map<ObjectHandle, ObjectHandle>.fromEntries(
            new List<MapEntry<ObjectHandle, ObjectHandle>>.generate(
                ...
                (_) => new MapEntry<ObjectHandle, ObjectHandle>(
                    reader.readPackedObject() as ObjectHandle,
                    reader.readPackedObject() as ObjectHandle)));
        break;
      case ConstTag.kMap:
        type = reader.readPackedObject() as ObjectHandle?;
        value = reader.readPackedMap<ObjectHandle>();
        break;
      case ConstTag.kList:
        type = reader.readPackedObject() as ObjectHandle?;
      ...
      case ConstTag.kMap:
        {
          ...
          for (int i = 0; i < entries.length; ++i) {
            entries[i].forEach((ObjectHandle key, ObjectHandle? value) {
      ...
      case ConstTag.kMap:
        {
          final entries = value as List<Map<ObjectHandle, ObjectHandle?>>;
          return _hashCode =
              _combineHashes(type.hashCode, listHashCode(entries));
        }
        break;
      case ConstTag.kList:
        {
...
class ObjectTable implements ObjectWriter, ObjectReader {
  ...
  late List<ObjectHandle?> _indexTable;
  ...
  List<ObjectHandle?> getHandles(List<Node> nodes) {
    ...
    final handles = [
      for (int i = 0; i < nodes.length; ++i) getHandle(nodes[i])
    ];
    return handles;
  }

  List<Map<ObjectHandle, ObjectHandle?>> getMapHandles(
      List<ConstantMapEntry> entries) {
    if (entries.length == 0) {
      return <Map<ObjectHandle, ObjectHandle?>>[];
    }
    // final handles = new List<Map<ObjectHandle, ObjectHandle>>.filled(
    //     entries.length, {getHandle(entries[0].key) as ObjectHandle : getHandle(entries[0].value) as ObjectHandle});
    // for (int i = 0; i < entries.length; ++i) {
    //   handles[i] = {getHandle(entries[i].key) as ObjectHandle : getHandle(entries[i].value) as ObjectHandle};
    // }
    final handles = [
      for (int i = 0; i < entries.length; ++i)
        {getHandle(entries[i].key)!: getHandle(entries[i].value)}
    ];
    return handles;
  }
  ...
  ObjectHandle getPublicNameHandle(String? name) {
    ...
    if (handle == null) {
      handle =
          getOrAddObject(new _PublicNameHandle(name!)) as _PublicNameHandle;
  ...
  ObjectHandle getNameHandle(Library? library, String name) {
    ...
    return getOrAddObject(
        new _PrivateNameHandle(libraryHandle as _LibraryHandle, name));
  }
  ...
  List<_PublicNameHandle> getPublicNameHandles(List<String> names) {
    ...
    final handles = new List<_PublicNameHandle>.filled(
        names.length, getPublicNameHandle(names[0]) as _PublicNameHandle);
  ...
  List<ObjectHandle> getConstStringHandles(List<String> values) {
    ...
    final handles = [
      for (int i = 0; i < values.length; ++i) getConstStringHandle(values[i])
    ];
    return handles;
  }
  ...
  ObjectHandle getTopLevelClassHandle(Library library) {
    ...
    return getOrAddObject(
        new _ClassHandle(libraryHandle as _LibraryHandle, name as NameHandle));
  }
  ...
  ObjectHandle getMemberHandle(Member member,
    ...
    return getOrAddObject(new _MemberHandle(classHandle as _ClassHandle,
        nameHandle as NameHandle, isField, isConstructor));
  }
  ...
  ObjectHandle? getTypeArgumentsHandle(List<DartType>? typeArgs) {
    ...
    final handles = [
      for (int i = 0; i < typeArgs.length; ++i)
        getHandle(typeArgs[i]) as TypeHandle
    ];
    ...
    if (namedArguments.isNotEmpty) {
      argNames = [
        for (int i = 0; i < namedArguments.length; ++i)
          getPublicNameHandle(namedArguments[i].name) as _PublicNameHandle
      ];
    }
  ...
  ObjectHandle getScriptHandle(Uri uri, SourceFile? source) {
    ...
    _ScriptHandle handle =
        getOrAddObject(new _ScriptHandle(uriHandle, source)) as _ScriptHandle;
  ...
  List<NameAndType> getTypeParameterHandles(List<TypeParameter> typeParams) {
    ...
    final namesAndBounds = <NameAndType>[];
    for (TypeParameter tp in typeParams) {
      namesAndBounds.add(new NameAndType(
          getPublicNameHandle(tp.name) as NameHandle,
          getHandle(tp.bound) as TypeHandle));
    }
  ...
  void allocateIndexTable() {
    ...
    _indexTable = new List<ObjectHandle?>.filled(tableSize, null);
    int count = 0;
  ...
  ObjectTable.read(BufferedReader reader) {
    ...
    _indexTable = new List<ObjectHandle?>.filled(numEntries, null);
...
class _NodeVisitor extends Visitor<ObjectHandle?> {
  ...
  ObjectHandle visitLibrary(Library node) {
    ...
    return objectTable
        .getOrAddObject(new _LibraryHandle(uri as _ConstObjectHandle));
  }
  ...
  ObjectHandle visitClass(Class node) {
    ...
    final name = node.name.startsWith('_')
        ? objectTable.getOrAddObject(
            new _PrivateNameHandle(library as _LibraryHandle, node.name))
        : objectTable.getPublicNameHandle(node.name);
    return objectTable.getOrAddObject(
        new _ClassHandle(library as _LibraryHandle, name as NameHandle));
  }
  ...
  //     objectTable.getHandle(coreTypes.deprecatedNullType);
  ...
  ObjectHandle visitInterfaceType(InterfaceType node) {
    ...
    if (!hasInstantiatorTypeArguments(node.classNode)) {
      return objectTable.getOrAddObject(new _SimpleTypeHandle(
          classHandle as _ClassHandle?, node.nullability));
    }
    ...
    final result = objectTable.getOrAddObject(isRecursive
        ? new _RecursiveGenericTypeHandle(
            recursiveId,
            classHandle as _ClassHandle,
            typeArgsHandle as _TypeArgumentsHandle?,
            node.nullability)
        : new _GenericTypeHandle(classHandle as _ClassHandle,
            typeArgsHandle as _TypeArgumentsHandle, node.nullability));
  ...
  ObjectHandle visitFutureOrType(FutureOrType node) {
    ...
    ObjectHandle? typeArgsHandle =
        objectTable.getTypeArgumentsHandle(instantiatorArgs);
    final result = objectTable.getOrAddObject(new _GenericTypeHandle(
        classHandle as _ClassHandle,
        typeArgsHandle as _TypeArgumentsHandle,
        node.nullability));
    return result;
  ...
  ObjectHandle visitInstanceConstant(InstanceConstant node) =>
      objectTable.getOrAddObject(new _ConstObjectHandle(ConstTag.kInstance,
          node.fieldValues.map<ObjectHandle, ObjectHandle?>(
              (Reference fieldRef, Constant? value) {
        return new MapEntry(objectTable.getHandle(fieldRef.asField)!,
  ...
  ObjectHandle visitMapConstant(MapConstant node) =>
      objectTable.getOrAddObject(new _ConstObjectHandle(
          ConstTag.kMap,
          objectTable.getMapHandles(node.entries),
          objectTable.getHandle(new InterfaceType(coreTypes.mapClass,
              Nullability.legacy, <DartType>[node.keyType, node.valueType]))));
