...
import 'package:front_end/src/fasta/fasta_codes.dart' as fc
    show Code, MessageCode, Template, Message;
...
import 'generics.dart'
    show
        ...
        isUncheckedCall,
        isUncheckedCallLocal,
        isUncheckedClosureCall,
        isUncheckedClosureCallForDynamic,
        isUncheckedClosureCallForFunction,
        isUncheckedClosureCallForLocalFunction;
...
const fc.MessageCode messageBytecodeLimitExceededTooManyArguments =
    const fc.MessageCode("BytecodeLimitExceededTooManyArguments",
        problemMessage:
            r"""Dart bytecode limit exceeded: too many arguments.""");

const fc.Code<fc.Message Function(DartType _type, bool isNonNullableByDefault)>
    codeIllegalRecursiveType = const fc
        .Code<fc.Message Function(DartType _type, bool isNonNullableByDefault)>(
...
fc.Message _withArgumentsIllegalRecursiveType(
  ...
  return new fc.Message(codeIllegalRecursiveType,
      problemMessage:
          """Illegal recursive type '${type}'.""" + labeler.originMessages,
...
class BytecodeGenerator extends RecursiveVisitor<Null> {
  ...
  BytecodeGenerator(
      ...
      : this._internal(
            ...
            typeEnvironment,
            options,
            new StatefulStaticTypeContext.flat(typeEnvironment),
            dynamicdill);
  ...
  BytecodeGenerator._internal(
    ...
    directCallMetadata = component
        .metadata[DirectCallMetadataRepository.repositoryTag]
        ?.mapping as Map<Node, DirectCallMetadata>;
    ...
    inferredTypeMetadata = component
        .metadata[InferredTypeMetadataRepository.repositoryTag]
        ?.mapping as Map<Node, InferredType>?;
  ...
  ParameterDeclaration getParameterDeclaration(VariableDeclaration variable) {
    ...
    final lib =
        name!.startsWith('_') ? enclosingMember?.enclosingLibrary : null;
  ...
  Procedure get listFromLiteral => _listFromLiteral ??= libraryIndex.getMember(
      'dart:core', '_GrowableList', '_literal') as Procedure;
  ...
  Procedure get interpolateSingle => _interpolateSingle ??= libraryIndex
      .getMember('dart:core', '_StringBase', '_interpolateSingle') as Procedure;

  Procedure? _interpolate;
  Procedure get interpolate => _interpolate ??= libraryIndex.getMember(
      'dart:core', '_StringBase', '_interpolate') as Procedure;
  ...
  Field get closureDelayedTypeArguments => _closureDelayedTypeArguments ??=
      libraryIndex.getMember('dart:core', '_Closure', '_delayed_type_arguments')
          as Field;
  ...
  Procedure get futureValue => _futureValue ??=
      libraryIndex.getMember('dart:async', 'Future', 'value') as Procedure;
  ...
  Procedure get unsafeCast => _unsafeCast ??= libraryIndex.getTopLevelMember(
      'dart:_internal', 'unsafeCast') as Procedure;

  Procedure? _iterableIterator;
  Procedure get iterableIterator => _iterableIterator ??= libraryIndex
      .getMember('dart:core', 'Iterable', 'get:iterator') as Procedure;
  ...
  Procedure get iteratorCurrent => _iteratorCurrent ??= libraryIndex.getMember(
      'dart:core', 'Iterator', 'get:current') as Procedure;
  ...
  ClosureDeclaration getClosureDeclaration(LocalFunction node,
    ...
    for (var v in function.positionalParameters) {
      parameters.add(new NameAndType(
          objectTable.getPublicNameHandle(v.name) as NameHandle,
  ...
  void _genInstanceUsingSpecializedBytecode(Opcode opcode, Expression node) {
    switch (opcode) {
  ...
  bool _isUncheckedCallLocal(
          LocalFunctionInvocation node, Member? interfaceTarget) =>
      isUncheckedCallLocal(interfaceTarget, node, staticTypeContext) ||
      (inferredTypeMetadata != null &&
          inferredTypeMetadata![node]?.skipCheck == true);
  ...
  void _genInstanceCallLocal(
    ...
    final isDynamic = interfaceTarget == null;
    final isUnchecked = invocationKind != InvocationKind.getter &&
        _isUncheckedCallLocal(node, interfaceTarget);
    ...
    bool generated = false;
    if (invocationKind != InvocationKind.getter && !isDynamic && !isUnchecked) {
      final staticReceiverType = node.functionType.returnType;
      if (isInstantiatedInterfaceCall(interfaceTarget, staticReceiverType)) {
        final callCpIndex = cp.addInstantiatedInterfaceCall(
            invocationKind, interfaceTarget!, argDesc, staticReceiverType);
        asm.emitInstantiatedInterfaceCall(callCpIndex, totalArgCount);
  ...
  void visitInstanceInvocation(InstanceInvocation node) {
    ...
    final Opcode? opcode =
        recognizedMethods.specializedBytecodeForInstance(node);
    if (dynamicdill) {
      if (opcode != null) {
        _genInstanceUsingSpecializedBytecode(opcode, node);
        return;
      }
    } else {
  ...
  void visitDynamicSet(DynamicSet node) {
    ...
    if (directCall != null) {
      final isDynamicForwarder = (null == null);
      final isUnchecked = _isUncheckedCall(node, null, node.receiver);
      _genDirectCall(directCall.target, argDesc, numArguments,
          isSet: true,
          isDynamicForwarder: isDynamicForwarder,
          isUnchecked: isUnchecked,
      ...
      _genInstanceCall(node, InvocationKind.setter, null, node.name,
  ...
  void visitFunctionTearOff(FunctionTearOff node) {
    ...
    if (directCall != null) {
      if (directCall.checkReceiverForNull) {
        ...
        asm.emitPush(receiverTemp);
        asm.emitNullCheck(
            cp.addSelectorName(Name('call'), InvocationKind.getter));
      }
      _genDirectCall(directCall.target, argDesc, 1, isGet: true, node: node);
    } else {
      _genInstanceCall(node, InvocationKind.getter, null, Name('call'),
  ...
  void visitDynamicGet(DynamicGet node) {
    ...
    if (directCall != null) {
      ...
      _genInstanceCall(node, InvocationKind.getter, null, node.name,
          node.receiver, 1, argDesc);
    }
  }
  ...
  void visitEqualsCall(EqualsCall node) {
    final directCall =
        directCallMetadata != null ? directCallMetadata[node] : null;
    final Opcode? opcode = recognizedMethods.specializedBytecodeForEqauls(node);
    if (dynamicdill) {
      if (opcode != null) {
        _genInstanceUsingSpecializedBytecode(opcode, node);
        return;
    ...
    List<Expression> arg = <Expression>[];
    arg.add(node.right);
    final args = new Arguments(arg);
    final name = new Name('==');
    final totalArgCount = 2;
    if (totalArgCount >= argumentsLimit) {
      throw new TooManyArgumentsException(node.fileOffset);
    }
    ...
    if (directCall != null && directCall.checkReceiverForNull) {
      final int receiverTemp = locals.tempIndexInFrame(node);
      _genArguments(node.left, args, storeReceiverToLocal: receiverTemp);
      asm.emitPush(receiverTemp);
      asm.emitNullCheck(cp.addSelectorName(name, InvocationKind.method));
    } else {
      _genArguments(node.left, args);
    }
    ...
    if (directCall != null) {
      final isDynamicForwarder = (interfaceTarget == null);
      final isUnchecked = _isUncheckedCall(node, interfaceTarget, node.left);
      _genDirectCall(directCall.target, argDesc, totalArgCount,
          isDynamicForwarder: isDynamicForwarder,
          isUnchecked: isUnchecked,
          node: node);
    } else {
      _genInstanceCall(node, InvocationKind.method, interfaceTarget, name,
          node.left, totalArgCount, argDesc);
    }
  }
  ...
  void visitEqualsNull(EqualsNull node) {
    final directCall =
        directCallMetadata != null ? directCallMetadata[node] : null;
    final Opcode? opcode = Opcode.kEqualsNull;
    if (dynamicdill) {
      if (opcode != null) {
        _generateNode(node.expression);
        asm.emitSpecializedBytecode(opcode);
  ...
  void visitFunctionInvocation(FunctionInvocation node) {
    final directCall =
        directCallMetadata != null ? directCallMetadata[node] : null;
    final Opcode? opcode =
        recognizedMethods.specializedBytecodeForFunction(node);
    if (dynamicdill) {
      if (opcode != null) {
        _genInstanceUsingSpecializedBytecode(opcode, node);
        return;
    ...
    final Procedure? interfaceTarget = null;
    if (!(interfaceTarget == null ||
        interfaceTarget is Procedure && !interfaceTarget.isGetter)) {
      throw new UnsupportedOperationError(
          'Unsupported MethodInvocation with interface target ${interfaceTarget.runtimeType} $interfaceTarget');
  ...
  void visitLocalFunctionInvocation(LocalFunctionInvocation node) {
    final directCall =
        directCallMetadata != null ? directCallMetadata[node] : null;
    final Opcode? opcode =
        recognizedMethods.specializedBytecodeForLocalFunction(node);
    if (dynamicdill) {
      if (opcode != null) {
        _genInstanceUsingSpecializedBytecode(opcode, node);
        return;
    ...
    // Front-end guarantees that all calls with known function type
    // do not need any argument type checks.
    if (isUncheckedClosureCallForLocalFunction(
        node, staticTypeContext, options)) {
      final int receiverTemp = locals.tempIndexInFrame(node);
      _genArguments(VariableGet(node.variable), args,
          storeReceiverToLocal: receiverTemp);
      // Duplicate receiver (closure) for UncheckedClosureCall.
      asm.emitPush(receiverTemp);
      final argDescCpIndex = cp.addArgDescByArguments(args, hasReceiver: true);
      asm.emitUncheckedClosureCall(argDescCpIndex, totalArgCount);
    ...
    if (directCall != null && directCall.checkReceiverForNull) {
      final int receiverTemp = locals.tempIndexInFrame(node);
      _genArguments(null, args, storeReceiverToLocal: receiverTemp);
      asm.emitPush(receiverTemp);
      asm.emitNullCheck(cp.addSelectorName(node.name, InvocationKind.method));
    } else {
      _genArguments(null, args);
    ...
    if (directCall != null) {
      final isDynamicForwarder = (interfaceTarget == null);
      final isUnchecked = _isUncheckedCallLocal(node, interfaceTarget);
      _genDirectCall(directCall.target, argDesc, totalArgCount,
          isDynamicForwarder: isDynamicForwarder,
          isUnchecked: isUnchecked,
          node: node);
    } else {
      _genInstanceCallLocal(node, InvocationKind.method, interfaceTarget,
          node.name, totalArgCount, argDesc);
    }
  }
  ...
  void visitDynamicInvocation(DynamicInvocation node) {
    final directCall =
        directCallMetadata != null ? directCallMetadata[node] : null;
    final Opcode? opcode =
        recognizedMethods.specializedBytecodeForDynamic(node);
    if (dynamicdill) {
      if (opcode != null) {
        _genInstanceUsingSpecializedBytecode(opcode, node);
        return;
  ...
  visitStaticGet(StaticGet node) {
    ...
    if (target is Field) {
      ...
      if (target.isGetter) {
        ...
        asm.emitPushConstant(
            cp.addObjectRef(new StaticTearOffConstant(target)));
      }
    } else {
