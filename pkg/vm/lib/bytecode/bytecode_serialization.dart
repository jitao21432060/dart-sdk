...
class BufferedWriter {
  ...
  Uint8List getContents() {
    if (_buffers == null) {
      return new Uint8List.view(_currentBuffer!.buffer,
  ...
  void appendUint8List(Uint8List src) {
    if (src.length < avoidCopyingSize) {
      ...
      _currentBuffer!
          .setRange(_currentLength, _currentLength + src.length, src);
  ...
  void appendWriter(BufferedWriter other) {
    ...
    if (remainingBytes >= minRemainderToKeep &&
        remainingBytes >
            (other._currentBuffer!.length - other._currentLength)) {
  ...
  @pragma('vm:prefer-inline')
  void writePackedMap(List<Map<BytecodeObject, BytecodeObject?>> objects) {
    writePackedUInt30(objects.length);
    for (var obj in objects) {
      obj.forEach((BytecodeObject key, BytecodeObject? value) {
        writePackedObject(key);
...
class BufferedReader {
  ...
  List<Map<T, T>> readPackedMap<T extends BytecodeObject>() {
    final int len = readPackedUInt30();
    final list = new List<Map<T, T>>.filled(len, 0 as Map<T, T>);
    for (int i = 0; i < len; ++i) {
      list[i] = {readPackedObject() as T, readPackedObject() as T} as Map<T, T>;
    }
    return list;
  }
  ...
  List<T>? readPackedList<T extends BytecodeObject?>() {
    final int len = readPackedUInt30();
    final list = [for (int i = 0; i < len; ++i) readPackedObject() as T];
    return list;
  }
...
class StringTable implements StringWriter, StringReader {
  ...
  StringTable.read(BufferedReader reader) {
    ...
    final List<int> oneByteEndOffsets =
        new List<int>.filled(numOneByteStrings, 0);
